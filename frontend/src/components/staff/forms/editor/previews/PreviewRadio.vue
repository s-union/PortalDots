...
<template>
  ...
  <div v-if="options.length > 0" class="grid gap-2">
    <label v-for="option in options" :key="option" class="flex items-center gap-3 text-base text-body">
  ...
  <div v-else class="empty-option">
    ...
    <p class="mt-1 text-base text-muted">選択肢を1つ以上入力してください。</p>
  </div>
</template>
