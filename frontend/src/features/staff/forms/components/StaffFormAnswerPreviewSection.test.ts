...
vi.mock("@/features/staff/forms/api", async () => {
    const actual = await vi.importActual<typeof import("@/features/staff/forms/api")>(
        "@/features/staff/forms/api",
    );
    return {
        ...actual,
        buildStaffFormUploadDownloadUrl: (formId: string, uploadId: string) =>
            `/download/${formId}/${uploadId}`,
    };
});
