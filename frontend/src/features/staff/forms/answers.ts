...
export type StaffManagedFormAnswerSummary = {
    id: string;
...
export type StaffFormAnswersIndex = {
...
export type StaffManagedFormAnswerDetail = {
...
type MutateStaffFormAnswerPayload = {
    circleId: string;
    body: string;
...
export async function createStaffFormAnswer(
    formId: string,
    ...
    return $api.mutationData(
        ...
        (value) =>
            parseWithSchema(
                z.object({ answer: staffManagedFormAnswerSummarySchema }),
                value,
                "staff form answer",
            ).answer,
        {
...
export function useUpdateStaffFormAnswerMutation(
    formId: MaybeRefOrGetter<string>,
    answerId: MaybeRefOrGetter<string>,
) {
    const queryClient = useQueryClient();
...
export function useDeleteStaffFormAnswerMutation(formId: MaybeRefOrGetter<string>) {
    ...
    return useMutation({
        mutationFn: async (answerId: string) =>
            deleteStaffFormAnswer(toValue(formId), answerId, sessionStore.csrfToken),
        onSuccess: async () => {
            await queryClient.invalidateQueries({
                queryKey: ["staff", "forms", toValue(formId), "answers"],
            });
...
export function buildStaffFormAnswerUploadDownloadUrl(
    formId: string,
    answerId: string,
    questionId: string,
) {
