...
export function useCreateStaffPlaceMutation() {
    ...
    return useMutation({
        mutationFn: async (payload: Omit<StaffPlace, "id">) =>
            createStaffPlace(payload, sessionStore.csrfToken),
        onSuccess: async () => {
...
export function useUpdateStaffPlaceMutation() {
    ...
    const sessionStore = useSessionStore();
    return useMutation({
        mutationFn: async (payload: StaffPlace) =>
            updateStaffPlace(payload, sessionStore.csrfToken),
        onSuccess: async () => {
            await queryClient.invalidateQueries({ queryKey: ["staff", "places"] });
        },
    });
