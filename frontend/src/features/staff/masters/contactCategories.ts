...
export function useUpdateStaffContactCategoryMutation() {
  ...
  const sessionStore = useSessionStore()
  return useMutation({
    mutationFn: async (payload: StaffContactCategory) => updateStaffContactCategory(payload, sessionStore.csrfToken),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['staff', 'contact-categories'] })
    }
  })
...
export function useDeleteStaffContactCategoryMutation() {
  ...
  const sessionStore = useSessionStore()
  return useMutation({
    mutationFn: async (categoryId: string) => deleteStaffContactCategory(categoryId, sessionStore.csrfToken),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ['staff', 'contact-categories'] })
    }
  })
