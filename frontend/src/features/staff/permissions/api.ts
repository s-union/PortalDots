...
import {
    parseWithSchema,
    staffPermissionDetailSchema,
    staffPermissionUserSummarySchema,
} from "@/lib/api/schema";
...
export type StaffPermissionUserSummary = {
    id: string;
    displayName: string;
    loginIds: string[];
    roles: string[];
...
type StaffPermissionsPagination = {
    page: number;
    pageSize: number;
};
...
export async function fetchStaffPermissions(pagination: StaffPermissionsPagination) {
    return $api.queryData(
        ...
        (value) =>
            parsePaginatedResult(value, parseStaffPermissionUserSummary, "staff permissions"),
        {
...
export function useStaffPermissionDetailQuery(
