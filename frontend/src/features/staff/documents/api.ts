...
export type MutateStaffDocumentPayload = {
    name: string;
    description: string;
    notes: string;
    isPublic: boolean;
...
export async function updateStaffDocument(
    documentId: string,
    payload: MutateStaffDocumentPayload,
    csrfToken: string,
) {
    const formData = new FormData();
    formData.set("name", payload.name);
    ...
    const response = await putMultipart(
        `/staff/documents/${encodePathSegment(documentId)}`,
        formData,
        csrfToken,
    );
    const raw = await response.text();
...
export function useStaffDocumentDetailQuery(
...
export function useCreateStaffDocumentMutation() {
    ...
    return useMutation({
        mutationFn: async (payload: MutateStaffDocumentPayload) =>
            createStaffDocument(payload, sessionStore.csrfToken),
        onSuccess: async () => {
            await Promise.all([
