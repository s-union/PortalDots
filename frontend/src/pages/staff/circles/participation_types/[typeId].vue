...
const settingsRoute = computed(() => `/staff/circles/participation_types/${encodeURIComponent(typeId.value)}`)
    ...
    <form v-else-if="detailQuery.data.value" class="space-y-6" @submit.prevent="handleSave">
      ...
      <SettingsSection id="circles-section" title="この参加種別に紐づく企画">
        <SurfaceHeader>
          ...
          <template #description> この参加種別に登録された企画を確認できます。 </template>
          <template #actions>
            <a
          ...
          <table class="min-w-full divide-y divide-border text-sm">
            <thead class="bg-surface-light text-left text-muted-2">
              <tr>
                ...
                <th class="px-5 py-3 font-medium">受理状況</th>
                <th class="px-5 py-3 font-medium">使用場所</th>
            ...
            <tbody class="divide-y divide-border">
              <tr v-for="circle in circlesQuery.data.value?.items" :key="circle.id">
                ...
                <td class="px-5 py-4">
                  <StatusBadge :tone="statusTone(circle.status)" size="sm">
                    {{ statusLabel(circle.status) }}
                  </StatusBadge>
                </td>
                <td class="px-5 py-4">
                  <div class="flex flex-wrap gap-1">
                    <StatusBadge v-for="place in circle.places" :key="place" tone="muted" size="sm">
                      {{ place }}
                    </StatusBadge>
                    <span v-if="circle.places.length === 0" class="text-muted">-</span>
                  </div>
                </td>
