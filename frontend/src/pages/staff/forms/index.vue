...
function resolveSortValue(row: Record<string, unknown>, key: StaffFormSortKey) {
  if (key === 'formNumber') return String(row.formNumber ?? '0').padStart(10, '0')
  if (key === 'isPublic') return row.isPublic ? '1' : '0'
  if (key === 'maxAnswers') return String(row.maxAnswers ?? 0).padStart(10, '0')
...
function matchesFilterQuery(row: Record<string, unknown>, query: { keyName: string; operator: string; value: string }) {
  ...
  if (query.keyName === 'isPublic') {
    ...
    if (query.operator === '=') return row.isPublic === expected
    if (query.operator === '!=') return row.isPublic !== expected
  }
