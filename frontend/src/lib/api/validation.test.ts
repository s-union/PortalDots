...
import { parseValidationError, extractValidationMessage, unwrapValidationError } from './validation'
...
describe('parseValidationError', () => {
  ...
  it('throws when input is not a valid validation error', () => {
    expect(() => parseValidationError(null, 'circle')).toThrow('Invalid circle validation error')
...
describe('extractValidationMessage', () => {
  it('returns fallback when error is not a validation error', () => {
    expect(extractValidationMessage(new Error('plain'), 'エラーが発生しました')).toBe('エラーが発生しました')
