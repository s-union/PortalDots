...
@section('content')
    ...
    <app-container>
        ...
        @if ($can_register)
            @if(empty($circle))
                <list-view>
                    ...
                    @else
                        @if (!$my_group->hasSubmitted())
                            @if($my_group->canSubmit() && Auth::user()->isLeaderInGroup($my_group))
                        ...
                        @else
                            <list-view-empty icon-class="fa fa-check" text="理大祭参加登録を提出しました！">
                                <p>
                                    理大祭参加登録を提出しました。
                                    @if (Auth::user()->isLeaderInGroup($my_group))
                                        <br>続いて、企画参加登録を行ってください。
                                    @endif
                                </p>
                            </list-view-empty>
                        @endif
        ...
        @if (Gate::allows('circle.create'))
            @if(empty($circle))
                <list-view>
                    <template v-slot:title>第１次企画参加登録</template>
                    <template v-slot:description>
                        受付期間 : @datetime($circle_custom_form->open_at)〜@datetime($circle_custom_form->close_at)
                    </template>
                    @if (!Auth::check())
                    ...
                    @else
                        @if($register_group_before_submitting_circle)
                            @include('includes.circle_with_group_list_view_item_with_status', ['group' => $my_group])
                        @else
                ...
                <list-view>
                    ...
                    <list-view-card>
                        <dl>
                            ...
                            @if ($my_group->circle() && $my_group->circle()->isPending())
                                @php
                                    $attendance_fee = ($my_group->food_booth + $my_group->seller_booth) * 12000
                                        + $my_group->exh_seller_booth * 10000 + $my_group->exh_booth * 7000;
                                @endphp
                                <dt>企画参加登録費の合計</dt>
        ...
        @if (Auth::check() && isset($circle) && !$register_group_before_submitting_circle)
            <list-view>
