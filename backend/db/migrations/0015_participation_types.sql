...
CREATE TABLE IF NOT EXISTS participation_types (
    ...
    form_id text NOT NULL UNIQUE REFERENCES forms(id) ON DELETE CASCADE,
    created_at timestamptz NOT NULL DEFAULT now(),
    updated_at timestamptz NOT NULL DEFAULT now()
);

ALTER TABLE circles
    ADD COLUMN IF NOT EXISTS participation_type_id text REFERENCES participation_types(id) ON DELETE SET NULL;
