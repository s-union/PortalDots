...
func TestArchiveEntryPath(t *testing.T) {
	...
	t.Run("contains circle and question names", func(t *testing.T) {
		t.Parallel()
		got := archiveEntryPath("食品", "食品", "パンフレット用データ", "資料.pdf")
		want := "食品/食品_パンフレット用データ_資料.pdf"
		if got != want {
	...
	t.Run("cannot escape archive layout", func(t *testing.T) {
		t.Parallel()
		got := archiveEntryPath("食品", "../../エスケープ", "質問\x00名", "../../../evil.pdf")
...
func TestStaffFormAnswerUploadsZIPEntryNames(t *testing.T) {
	...
	wantEntry := "デモ企画A/デモ企画A_Test upload_レイアウト図.pdf"
